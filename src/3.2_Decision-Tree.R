...
# - Using "rpart"
# - 82.91% Accuracy on Training Set
# - 85.31% Accuracy on Validation Set (Might be too high)
# - 83.39% Accuracy on the Whole Titanic Set (Better than based only on Gender)

modelFit <- train(Survived ~ ., method = "rpart", data = titanic_train)
modelFit$finalModel
...
write_csv(result_final, "../Data/output/3.2_Decision-Tree.csv")
